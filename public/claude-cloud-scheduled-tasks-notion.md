...
| 方法 | 構築の手軽さ | メンテナンス | コスト | 必要スキル |
|------|:-----------:|:----------:|:-----:|-----------|
| Notion AI | ◎ 設定のみ | ◎ 不要 | △ 月$20/member | 不要 |
| GitHub Actions + Claude API | △ コード開発 | × コード修正 | ◎ API従量課金 | 開発力 |
| GAS + AI API | △ コード開発 | × コード修正 | ◎ API従量課金 | 開発力 |
| Zapier / Make | ○ GUI設定 | ○ GUI再設定 | △ 月$9〜20 | 低 |
| **Cloud Scheduled Tasks** | **○ 自然言語** | **◎ プロンプト修正** | **◎ Pro/Max内 ※** | **Git基本操作** |

※ Cloud Scheduled TasksはClaude Pro/Maxプランのサブスクリプションに含まれており、追加課金なしで利用できます。筆者はMaxプラン契約者のため、この記事で紹介する構成は追加費用ゼロで実現しています。

### Notion AI

一番楽です。Notion内で完結し、セットアップもほぼ不要。ただし、完全なAI機能（自動要約含む）を使うには[Businessプラン（月$20/member）](https://www.notion.com/ja/product/ai)が必要です。「これをやめたい」が今回の出発点なので、選択肢から外します。
